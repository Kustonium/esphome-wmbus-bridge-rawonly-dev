# Fix: CC1101 lost register writes, and stopped listening on a sync word that is never sent
...
- **The `0x54CD` sync cycle is gone.** Mode T and mode C share one sync word, `0x543D`. In a mode C telegram the `0x54CD` that follows is the frame-format marker and arrives as *data* - which is exactly why `packet.cpp` strips it as `WMBUS_MODE_C_SUFIX_LEN`. Arming the radio on `0x54CD` therefore never caught a C1 frame from its start; it only spent one listening window in four on a sync word that is not transmitted as one. `SYNC0` is now written once and left alone, as in both reference drivers.
...
- **New field `chip_not_ready` in the `CC1101 debug status` line**, counting transactions the chip answered with `CHIP_RDYn` still high. A non-zero value points at supply voltage, wiring length or SPI clock - not at the decoder.
...
- **Cykl sync `0x54CD` zniknął.** Tryb T i tryb C dzielą jeden sync word, `0x543D`. W telegramie trybu C następujące po nim `0x54CD` to znacznik formatu ramki i przychodzi jako *dane* - dokładnie dlatego `packet.cpp` odcina je jako `WMBUS_MODE_C_SUFIX_LEN`. Uzbrajanie radia na `0x54CD` nigdy więc nie łapało ramki C1 od początku; marnowało tylko co czwarte okno nasłuchu na sync word, który nie jest w ten sposób nadawany. `SYNC0` jest teraz zapisywany raz i nietykany, tak jak w obu sterownikach referencyjnych.
...
- **Nowe pole `chip_not_ready` w linii `CC1101 debug status`**, liczące transakcje, na które układ odpowiedział z `CHIP_RDYn` wciąż wysokim. Wartość niezerowa wskazuje na zasilanie, długość okablowania albo zegar SPI - nie na dekoder.
