...
CONF_AGC_FIRST_POWER_THRESHOLD = "agc_first_power_threshold"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_AGC_FIRST_POWER_THRESHOLD, default=-1): cv.int_range(min=-1, max=255),
...
def _validate_radio_pins(config):
    ...
    if radio_type != "SX1262" and config.get(CONF_AGC_FIRST_POWER_THRESHOLD, -1) != -1:
        raise cv.Invalid("agc_first_power_threshold is only valid for radio_type: SX1262.")
...
async def to_code(config):
    ...
    if config[CONF_RADIO_TYPE] == "SX1262":
        ...
        cg.add(radio_var.set_agc_first_power_threshold(config.get(CONF_AGC_FIRST_POWER_THRESHOLD, -1)))
